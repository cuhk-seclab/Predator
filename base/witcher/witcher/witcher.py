...
#            dictionary_vars.append(b"%s'(&" % inputvar.encode("utf-8"))
    ...
    def filter_XSS_suspects(self, file_path):
        ...
        XSS
        """
        xss_patterns = ['<script>', 'javascript:', "onload='", "onmouseover='"]
        filtered_lines = []
        with open(file_path, 'r', errors='ignore') as file:
            for line in file:
                lower_line = line.lower()
                if 'alert(290363)' in lower_line and any(xss in lower_line for xss in xss_patterns):
            ...
            for trial_index in range(0, nbr_trials):
                ...
                for refuzz_index in range(0, nbr_refuzzes):
                    ...
                    for target in targets[target_start: target_end]:
                        ...
                        out_files = glob.glob(os.path.join(self.work_dir, '*.out'))
                        print(f"Filtering XSS suspects...")
                        for file_name in out_files:
                            self.filter_XSS_suspects(file_name)
